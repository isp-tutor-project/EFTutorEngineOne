...
import { TTextEvent } 		from "./events/TTextEvent";
...
export class TCursorProxy extends TRoot
{
	...
	curObject:TObject = null;
	actObject:TObject = null;
...
//***************** Automation *******************************		
	...
	{			
		...
		{
			this.stage.addEventListener(TMouseEvent.MOUSE_MOVE  , this.liveMouseMove);
			this.stage.addEventListener(TMouseEvent.MOUSE_DOWN  , this.liveMouseDown);
			this.stage.addEventListener(TMouseEvent.MOUSE_UP    , this.liveMouseUp);		
			this.stage.addEventListener(TMouseEvent.DOUBLE_CLICK, this.liveMouseDblClick);		
		}
		// Input is being driven by log input
		//
		else if(sMode == TCursorProxy.WOZREPLAY)
		{
			this.stage.removeEventListener(TMouseEvent.MOUSE_MOVE  , this.liveMouseMove);
			this.stage.removeEventListener(TMouseEvent.MOUSE_DOWN  , this.liveMouseDown);
			this.stage.removeEventListener(TMouseEvent.MOUSE_UP    , this.liveMouseUp);						
			this.stage.removeEventListener(TMouseEvent.DOUBLE_CLICK, this.liveMouseDblClick);		
		}			
	...
	public decodeTarget(baseObj:any, objArray:Array<any> ) : TObject
	{
		let tmpObject:TObject = null;
	...
	public playBackAction(wozEvt:any ) : void
	{
		...
		if(wozEvt.CEFMouseEvent != undefined)
		{
			...
			if(this.fSparklerTest)
			{
				...
				if(wozEvt.CEFMouseEvent.CEFEvent.type.toString() == TMouseEvent.WOZMOVE) this.fSparklerDrag = true;
			}

			if((wozEvt.CEFMouseEvent.CEFEvent.type.toString() == TMouseEvent.WOZDOWN) && this.fSparkler)
			{
			...
			if((wozEvt.CEFMouseEvent.CEFEvent.type.toString() == TMouseEvent.WOZUP) && this.fSparklerDrag)
			...
			if(tarObject)
			{
				...
				let evt:TMouseEvent = new TMouseEvent(tarObject.objID, wozEvt.CEFMouseEvent.CEFEvent.type, wozEvt.bubbles, wozEvt.cancelable, wozEvt.stageX, wozEvt.stageY,wozEvt.nativeEvent, wozEvt.pointerID, wozEvt.primary, wozEvt.rawX, wozEvt.rawY);
		...
		{	
			...
			if(wozEvt.CEFTextEvent.CEFEvent.type == TTextEvent.WOZINPUTTEXT)
			{
				...
				if(tarObject)
				{
					...
					let tEvt:TTextEvent = new TTextEvent(tarObject.objID, wozEvt.CEFTextEvent.CEFEvent.type, wozEvt.CEFTextEvent.index1, wozEvt.CEFTextEvent.index2, wozEvt.CEFTextEvent.text, true, false );						
...
//********************** Playback 
	...
	public replayEvent(xEvt:any ) : void
	{
		...
		{
			this.fSparklerTest = false;
			if(xEvt.CEFEvent.type.toString() == TMouseEvent.WOZMOVE) this.fSparklerDrag = true;
		}

		if((xEvt.CEFEvent.type.toString() == TMouseEvent.WOZDOWN) && this.fSparkler)
		{
			this.fSparklerDrag = false;
			this.fSparklerTest = true;
			this.Ssparkle.gotoAndPlay(2);			
		...
		if((xEvt.CEFEvent.type.toString() == TMouseEvent.WOZUP) && this.fSparklerDrag)
															this.Ssparkle.gotoAndPlay(10);			
		//
		//*** Click indicator *************
...
//							CUtil.trace("test point");
			...
			let evt:TMouseEvent = new TMouseEvent(tarObject.objID, xEvt.CEFEvent.type, xEvt.bubbles, xEvt.cancelable, xEvt.stageX, xEvt.stageY,xEvt.nativeEvent, xEvt.pointerID, xEvt.primary, xEvt.rawX, xEvt.rawY);
	...
	public replayEventB(xEvt:any ) : void
	{
		...
		if(tarObject)
		{
			...
			{
				case TMouseEvent.WOZMOVE:				// Ignore Event
							return;			
							
				case TMouseEvent.WOZOUT:					// Use "Current" Object
							tarObject = this.curObject;			
							break;
							
				case TMouseEvent.WOZOVER:				// Set Current Object
							this.curObject = tarObject;			
							break;
							
				case TMouseEvent.WOZUP:					// Direct "Up" to previous "Down" target
							tarObject = this.actObject;	
							break;
							
				case TMouseEvent.WOZDOWN:				// remember Down target object
							this.actObject = this.curObject;
							tarObject = this.curObject;
							break;
							
				case TMouseEvent.WOZCLICKED:				// Direct "Click" to previous "Down" target
							tarObject = this.actObject;	
							break;								
							
				case TMouseEvent.WOZDBLCLICK:			// Direct "Double Click" to previous "Down" target
							tarObject = this.actObject;	
							break;								
			}
...
//***********  Live behaviors
	...
	public liveMouseMove(evt:TMouseEvent)
	{
		let evtMove:TMouseEvent;
		...
		if(fUpdate)
		{
			...
			if(this.curObject)
			{
				...
				//
				evtMove = new TMouseEvent("none", TMouseEvent.WOZMOVE, evt.bubbles, evt.cancelable, evt.stageX, evt.stageY, evt.nativeEvent, evt.pointerID, evt.primary, evt.rawX, evt.rawY);
	...
	public liveMouseDown(evt:TMouseEvent)
	{
		let locX:number; 
		let locY:number; 
		...
		if(this.curObject)
		{
			...
			//			
			let evtDown:TMouseEvent = new TMouseEvent(this.curObject.objID, TMouseEvent.WOZDOWN, evt.bubbles, evt.cancelable, evt.stageX, evt.stageY, evt.nativeEvent, evt.pointerID, evt.primary, evt.rawX, evt.rawY);
	...
	public liveMouseUp(evt:TMouseEvent)
	{
		...
		if(this.actObject)
		{
			let evtUp:TMouseEvent = new TMouseEvent(this.actObject.objID, TMouseEvent.WOZUP, evt.bubbles, evt.cancelable, evt.stageX, evt.stageY, evt.nativeEvent, evt.pointerID, evt.primary, evt.rawX, evt.rawY);
			...
			if(this.actObject == this.curObject)		
			{
				...
				let evtClicked:TMouseEvent = new TMouseEvent(this.curObject.objID, TMouseEvent.WOZCLICKED, evt.bubbles, evt.cancelable, evt.stageX, evt.stageY, evt.nativeEvent, evt.pointerID, evt.primary, evt.rawX, evt.rawY);
	...
	public liveMouseDblClick(evt:TMouseEvent)
	{
		let locX:number; 
		let locY:number; 
		...
		if(this.curObject)
		{
			...
			//
			let evtDblClick:TMouseEvent = new TMouseEvent(this.curObject.objID, TMouseEvent.WOZDBLCLICK, evt.bubbles, evt.cancelable, evt.stageX, evt.stageY, evt.nativeEvent, evt.pointerID, evt.primary, evt.rawX, evt.rawY);
...
//***********  Utility Functions
	...
	public stateHelper(tarObj:TObject ) : boolean
	{
	...
	public hitTestCoord(locX:number, locY:number ) : TObject
	{
	...
	public hitTestMouse(evt:TMouseEvent)
	{
		let hitObj:TObject;
	...
	private updateCurrentObject(evt:TMouseEvent, hitObj:TObject)
	{
		...
		else
		{
			if(this.curObject)
			{
				...
				let evtOut:TMouseEvent = new TMouseEvent(this.curObject.objID, TMouseEvent.WOZOUT, evt.bubbles, evt.cancelable, evt.stageX, evt.stageY, evt.nativeEvent, evt.pointerID, evt.primary, evt.rawX, evt.rawY);
			...
			{					
				...
				let evtOver:TMouseEvent = new TMouseEvent(this.curObject.objID, TMouseEvent.WOZOVER, evt.bubbles, evt.cancelable, evt.stageX, evt.stageY, evt.nativeEvent, evt.pointerID, evt.primary, evt.rawX, evt.rawY);
	...
	public isWOZObject(tObj:any) : TObject
	{
		...
		//
		if(!tObj || tObj instanceof TSceneBase)
							return null;
		
		// Hit test WOZ Objects - Exclude the cursor 
		//
		else if(tObj instanceof TObject)  
								return tObj;
