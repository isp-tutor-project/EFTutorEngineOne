...
//***************** Automation *******************************		
	...
	{								
		...
		// Enumerate the child objects - Adobe AnimateCC instantiates these as properties of the object 
		...
		for(propName of nonTObj) {
			...
			if(childObj instanceof DisplayObject)
			{
                ...
                // 
                //  NOTE: Object xnames may be overridden in their serialization (i.e. dataObj). 
                //        If that is the case we keep the override.  So if the sname does not start
                //        with the autogenerated signature we assume it has been overridden.
                // 
                if(!(childObj as any).xname || (childObj as any).xname.startsWith(CONST.XNAME_SIG)) {
